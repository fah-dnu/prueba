...
using DNU_ParabiliaAltaTarjetasAnonimas.BaseDatos;
...
namespace DNU_ParabiliaAltaTarjetasAnonimas.LogicaNegocio
{
    public class LNArchivo
    {
        ...
        private static string NombreNewRelic
        {
            ...
            get
            {
                string ClaveProceso = "PRALTATARJPARABILIA";
        ...
        private static bool ejecutaActivacion(ref List<Archivo> lstArchvio, ref StringBuilder sb, String claveColectiva, string tipoManufactura, string subproducto = null, bool subproductoValido = false, bool esCredito = false,string log="",string ip="")
        {
            ...
            foreach (var item in lstArchvio)
            {
                ...
                try
                {
                    ...
                    if (!esCredito)//string.IsNullOrEmpty(subproducto))
                    {
                                ...
                                try
                                {
                                    ...
                                    if (datosArchivo[1] != "00000")
                                    {
                                        ...
                                        if (resultado[0] == "00")
                                        {
                                            ...
                                            var tablaAltaCuentaCacao = DAOArchivo.AgregarMedioAcceso(cuentaCacao, idMaCACAO, tipoMARegreso, conn, transaccionSQL, subproducto);// DBProcesadorArchivo.strBDEscrituraAutorizador);DBProcesadorArchivo.strBDEscrituraAutorizador
...
# else
                                    ...
                                    else//ya no funciona
                                    {
                                        ...
                                        resultado[0] = "99";
                                        resultado[1] = "Cuenta incorrecta";


                                        ////agregar tarjeta Externos Credito
                                        //DataTable resultadoObtenerDatosTarjeta = DAOArchivo.resultadoObtieneDatosTarjetaExterna("", item.NumeroTarjeta, DBProcesadorArchivo.strBDEscrituraAutorizador);//
                                        //string respuestaDatosTarjeta = resultadoObtenerDatosTarjeta.Rows[0]["tipo"].ToString();
                                        //if (respuestaDatosTarjeta == "correcto")
                                        //{
                                        //    colectivacuentahabiente = resultadoObtenerDatosTarjeta.Rows[0]["cuentahabiente"].ToString();
                                        //    tipoManufactura = resultadoObtenerDatosTarjeta.Rows[0]["manufactura"].ToString();
                                        //    resultado = DAOArchivo.AltaTarjeta(item, claveColectiva, tipoManufactura, DBProcesadorArchivo.strBDEscrituraAutorizadorExerno, true, colectivacuentahabiente);
                                        //    if (resultado[0] == "00")
                                        //    {
                                        //        transaccionSQL.Commit();
                                        //    }
                                        ...
                                        //    resultado = new string[2];
                                        //    resultado[0] = "99";
                                        //    resultado[1] = "Error al obtener cuentahabiente";
                                        //}
                                    }
                                }
                    ...
                    else//credito
                    {
                        if (!subproductoValido)
                        {
                            resultado = new string[2];
                            resultado[0] = "99";
                            resultado[1] = "El subproducto no existe";
                        }
                        else
                        {
                            SqlConnection conn = new SqlConnection(DBProcesadorArchivo.strBDEscrituraAutorizador);
                            try
                            {
                                ...
                                using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                                {
                                    try
                                    {
                                        resultado = DAOArchivo.AltaTarjetaCredito(item, claveColectiva, tipoManufactura, subproducto, conn, transaccionSQL);
                                        if (resultado[0] == "00" || resultado[0] == "01" || resultado[0] == "02")
                                        {
                                            //cuando se unifiquen las bases esta validacion solo sera si es 00
                                            colectivacuentahabiente = resultado[2];
                                            DataTable resultadoAltaCorte = DAOArchivo.resultadoAltaCorte(subproducto, item.NumeroTarjeta, conn, transaccionSQL);
                                            codigoCreacionCorte = resultadoAltaCorte.Rows[0]["tipo"].ToString();
                                            if (codigoCreacionCorte == "correcto")
                                            {
                                                transaccionSQL.Commit();
                                            }
                                            else if (resultadoAltaCorte.Rows[0]["Codigo"].ToString() == "9085")
                                            {
                                                //cuando se unifiquen las bases esta parte no ira pues, pues nunca
                                                //debera exisitir ya un corte creado por lo que se tendraque quitar la validacion de codigo 9085

                                                codigoCreacionCorte = "correcto";
                                                transaccionSQL.Commit();
                                            }
                                            else
                                            {
                                                try
                                                {
                                                    resultado[1] = resultadoAltaCorte.Rows[0]["error"].ToString();
                                                }
                                                catch (Exception procAlta)
                                                {
                                                    resultado[1] = "error al generar el corte";
                                                    LogueoPrAltaTarjParabilia.Error("[" + ip + "] [ProcesarAltaTarjetas] [PROCESADORNOCTURNO] [" + log + "] [" + procAlta.Message + "]");
                                                }
                                                resultado[0] = "98";
                                                transaccionSQL.Rollback();
                                            }
                                    ...
                                    {
                                        transaccionSQL.Rollback();
                                        LogueoPrAltaTarjParabilia.Error("[" + ip + "] [ProcesarAltaTarjetas] [PROCESADORNOCTURNO] [" + log + "] [" + tran.Message + "]");
                        ...
                        if (codigoCreacionCorte == "correcto")//esta parte igual cuando se unifiquen la sbases ya no tendra que ir
                        {
                            //    resultado = DAOArchivo.AltaTarjetaExterna(item, claveColectiva, tipoManufactura, DBProcesadorArchivo.strBDEscrituraAutorizadorExerno, true, colectivacuentahabiente);
                        }
        ...
        internal static string LimpiarArchivo(String elArchivo, string path,string log,string ip)
        {
            ...
            using (StreamReader sr = new StreamReader(elArchivo, Encoding.UTF8))
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (Regex.Match(line, @"^[0-9]{8}[\s][0-9]{13,15}[\s]").Success)
                    {
                       ...
                       // {
                        ...
                        //    LogueoPrAltaTarjParabilia.Error("[" + ip + "] [ProcesarAltaTarjetas] [PROCESADORNOCTURNO] [" + log + "] [" + ex.Message + "]");
                        //}
                        var quintoimite = cuartolimite + 15;
