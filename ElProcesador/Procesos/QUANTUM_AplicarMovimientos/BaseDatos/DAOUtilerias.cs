...
namespace Executer.BaseDatos
{
   ...
   public int getIDColectivaFromMA(String MedioAcceso, String TipoCuenta,Int64 la) {
    ...
    public static List<Movimiento> ObtieneMomivientosNuevos()
    {

        SqlDataReader rsLosMovimientos = null;
        //CallableStatement spEjecutor = null;
        List<Movimiento> losMovimientos = new List<Movimiento>();

        try
        {
            SqlConnection connConsulta = BDAplicarMov.BDLectura;
            SqlParameter param;


            SqlCommand comando = new SqlCommand("EJECUTOR_ObtieneMovimientosNuevos", connConsulta);
            comando.CommandType = CommandType.StoredProcedure;
            


            rsLosMovimientos = comando.ExecuteReader();

            if (null != rsLosMovimientos)
            {

                while (rsLosMovimientos.Read())
                {

                    Movimiento unMovimiento = new Movimiento();

                    unMovimiento.Consecutivo = rsLosMovimientos["CONSECUTIVO"].ToString();
                    unMovimiento.ID_Cliente = rsLosMovimientos["ID_CLIENTE"].ToString();
                    unMovimiento.Tarjeta = rsLosMovimientos["TARJETA"].ToString();
                    unMovimiento.ID_Concepto_Evento = rsLosMovimientos["ID_CONCEPTO"].ToString();
                    unMovimiento.Importe = (Decimal)rsLosMovimientos["IMPORTE"];
                    unMovimiento.Observaciones = rsLosMovimientos["OBSERVACIONES"].ToString();
                    unMovimiento.ClaveCadenaComercial = rsLosMovimientos["CLAVECADENA"].ToString();
        ...
        {
            Logueo.Error("EJECUTOR:[ID_Poliza:" +  "] insertaSaldosEnPoliza():" + er.Message);
            throw er;
        }
        catch (Exception er)
        {
    ...
    public static void setMovimientoProcesado(Int64 ID_Movimiento, String laTarjeta)
    {
       ...
       int IdColectiva=0;
        ...
        try {
                        ...
                        SqlCommand comando = new SqlCommand("EJECUTOR_setMovimientoProcesado", BDAplicarMov.BDEscritura);
            comando.CommandType = CommandType.StoredProcedure;
            

            param = new SqlParameter("@ID_Movimiento", SqlDbType.VarChar);
            param.Value = ID_Movimiento;
            comando.Parameters.Add(param);

            param = new SqlParameter("@Tarjeta", SqlDbType.BigInt);
            param.Value = laTarjeta;
            comando.Parameters.Add(param);
