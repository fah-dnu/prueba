...
namespace Executer.BaseDatos
{
    ...
    private  Poliza insertaDetallesPoliza(ref Poliza laPoliza, SqlConnection connOperacion, SqlTransaction transaccionSQL) {
        ...
        try {
            ...
            foreach (DetallePoliza elDetalle in laPoliza.Detalles)
            {
                ...
                param = new SqlParameter("@Descripcion", SqlDbType.VarChar,99);
                param.Value = "-1";
                param.Direction = ParameterDirection.Output;
                comando.Parameters.Add(param);
                ...
                switch (resp)
                {
                    case 0:
                        break;
                    default:
                        Logueo.Error("EJECUTOR: insertaDetallesPoliza(): No se pudo insertar Detalle:[" + resp + "]");
    ...
    private List<Saldo> ObtieneSaldos(int idPoliza, SqlConnection connConsulta)
    {
