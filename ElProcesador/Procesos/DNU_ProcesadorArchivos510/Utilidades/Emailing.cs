...
namespace DNU_ProcesadorArchivos.Utilidades
{
    public class Emailing
    {
        ...
        static Emailing()
        {
            clienteSmtp = new SmtpClient(PNConfig.Get("PROCARCH510", "SMTPServer"), Int16.Parse(PNConfig.Get("PROCARCH510", "SMTPPort") ));

            //if (ConfigurationManager.AppSettings["SMTPUser"].Contains('@'))
            //   {

            clienteSmtp.Credentials = new NetworkCredential(PNConfig.Get("PROCARCH510", "SMTPUser") , PNConfig.Get("PROCARCH510", "SMTPPass"));
        ...
        public static int Send(string strTo, string strMessage, string strSubject, Attachment Atach, Attachment Atach2)
        {
            ...
            try
            {
                ...
                String smtpUser = PNConfig.Get("PROCARCH510", "SMTPUser");// Configuracion.Get(new Guid(ConfigurationManager.AppSettings["IDApplication"].ToString()), "SMTPUser").Valor;
                String smtpServer = PNConfig.Get("PROCARCH510", "SMTPServer"); //Configuracion.Get(new Guid(ConfigurationManager.AppSettings["IDApplication"].ToString()), "SMTPServer").Valor;
