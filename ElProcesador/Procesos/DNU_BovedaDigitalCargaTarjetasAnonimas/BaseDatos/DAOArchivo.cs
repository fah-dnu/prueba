...
namespace DNU_BovedaDigitalCargaTarjetasAnonimas.BaseDatos
{
    public class DAOArchivo
    {
        ...
        internal static DataTable AltaTarjeta(Archivo item, String claveColectiva, string tipoManufactura, String connString, bool tarjetaActiva = false, string colectivaCuentahabiente = null, SqlConnection connSQL = null, SqlTransaction transaccionSQL = null, string subproducto = null, int contador=0, Int64 idLote=0,string idSolicitud="",int totalTarjetas=0, string log="", string ip="")
        {
            ...
            try
            {
                ...
                string[] datosArchivo = item.RawData.Split('|');
                   ...
                   procedimientoAlmacenado = "BatchDigitalWarehouse_Lots_AddLots";// "Procnoc_Travel_AltaParabiliaAutomaico";
          

                // using (SqlConnection conLocal = new SqlConnection(connString))
                // {
                // using (
                SqlCommand command = new SqlCommand(procedimientoAlmacenado, connSQL);//)
                                                                                                      //    {
                command.CommandType = CommandType.StoredProcedure;
                ...
                //command.Parameters.Add(new SqlParameter("@ClaveColectiva", claveColectiva));
                command.Parameters.Add(new SqlParameter("@lot_key", idSolicitud));
               ...
               // command.Parameters.Add(new SqlParameter("@cuenta", item.Cuenta));
                command.Parameters.Add(new SqlParameter("@sub_bins_group_key", subproducto));
                command.Parameters.Add(new SqlParameter("@lot_type_key", "RES"));
                command.Parameters.Add(new SqlParameter("@manufacturing_type_key", tipoManufactura=="V"? tipoManufactura:"F"));
                command.Parameters.Add(new SqlParameter("@items", "1"));
                if(idLote!=0)
                command.Parameters.Add(new SqlParameter("@id_lot", idLote));
                ...
                paramSSNCuenta.ParameterName = "@cuentaTar";
                ...
                paramSSNCuenta.Size = 256;
                command.Parameters.Add(paramSSNCuenta);

                command.Parameters.Add(new SqlParameter("@totalItems", totalTarjetas));
                command.Parameters.Add(new SqlParameter("@sub_bin_key", item.NumeroTarjeta.Substring(0,10)));
                command.Parameters.Add(new SqlParameter("@numero", item.NumeroTarjeta.Substring(10, 5)));
                command.Parameters.Add(new SqlParameter("@dv", item.NumeroTarjeta.Substring(15, 1)));
                command.Parameters.Add(new SqlParameter("@user", "Batch"));
                ...
                sda.Fill(opcional);
                DataTable retorno = opcional.Tables.Count > 0 ? opcional.Tables[0] : null;
        ...
        public static DataTable validaExistenciaSubproducto(string subproducto, string conexion, string log, string ip)
        {
            ...
            try
            {
                ...
                using (SqlConnection conLocal = new SqlConnection(conexion))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("WsDigitalWarehouse_Bins_VerifySubBinGroup"))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Connection = conLocal;
