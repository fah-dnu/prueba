...
namespace DNU_ProcesadorT112.Utilidades
{
    public class Emailing
    {
        ...
        static Emailing()
        {
            clienteSmtp = new SmtpClient(PNConfig.Get("PROCESAT112", "SMTPServer"), Int16.Parse(PNConfig.Get("PROCESAT112", "SMTPPort") ));

            //if (ConfigurationManager.AppSettings["SMTPUser"].Contains('@'))
            //   {

            clienteSmtp.Credentials = new NetworkCredential(PNConfig.Get("PROCESAT112", "SMTPUser") , PNConfig.Get("PROCESAT112", "SMTPPass"));
        ...
        public static int Send(string strTo, string strMessage, string strSubject, Attachment Atach, Attachment Atach2)
        {
            ...
            try
            {
                ...
                String smtpUser = PNConfig.Get("PROCESAT112", "SMTPUser");// Configuracion.Get(new Guid(ConfigurationManager.AppSettings["IDApplication"].ToString()), "SMTPUser").Valor;
                String smtpServer = PNConfig.Get("PROCESAT112", "SMTPServer"); //Configuracion.Get(new Guid(ConfigurationManager.AppSettings["IDApplication"].ToString()), "SMTPServer").Valor;
