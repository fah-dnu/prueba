...
namespace DNU_ParabiliaAltaTarjetasNominales.LogicaNegocio
{
    public class LNAltaTarjetaNominal
    {
        ...
        {//
            ...
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Hubo un Cambio [" + elTipoCambio.ToString() + "] en el directorio: " + PNConfig.Get("ALTAEMPLEADOCACAO", "DirectorioEntrada") + " el se recibio el archivo : " + e.FullPath + "]");
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Espera a que se copie el archivo completo y lo libere el proceso de copiado]");
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [INICIO DE PROCESO DEL ARCHIVO:" + e.FullPath + "]");
        ...
        public void OcurrioError(Object sender, ErrorEventArgs e)
        {
            //string log = ThreadContext.Properties["log"].ToString();
            //string ip = ThreadContext.Properties["ip"].ToString();
            LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, OcurrioError, El evento de fileWatcher no inicio correctamente, Mensaje: " + e.GetException().Message + " TRACE: " + e.GetException().StackTrace + "]");
        }
        ...
        private void decodificarArchivo(string pPath, string pNomArchivo)
        {
            ...
            try
            {
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Inicia Validacion del Archivo: " + pPath + "" + idLog + "]");
                if (validarContenido(InfoArchivo))
                {
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Insertando informacion]");
                    ...
                    if (insertaDatos(pNomArchivo))
                    {
                        ...
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado Archivo " + InfoArchivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Procesados]");
                    }
                    else
                    {
                        ...
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Archivo " + InfoArchivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Erroneos]");
                    }
                }
                else if (validarContenidoCredito(InfoArchivo))
                {
                    if (insertaDatos(pNomArchivo, true, idLog))
                    {
                        string rutaFinal = directorio + "\\Procesados\\" + InfoArchivo.Name.Replace("EN_PROCESO_", "PROCESADO_");
                        moverArchivo(InfoArchivo.FullName, rutaFinal);
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Archivo " + InfoArchivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Procesados " + "[" + idLog + "]");
                    }
                    else
                    {
                        string pathArchivosInvalidos = directorio + "\\Erroneos\\" + InfoArchivo.Name.Replace("EN_PROCESO_", "PROCESADO_CON_ERRORES_");
                        moverArchivo(InfoArchivo.FullName, pathArchivosInvalidos);
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Archivo " + InfoArchivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Erroneos" + "[" + idLog + "]");
                    }
            ...
            catch (Exception ex)
            {
                string pathArchivosInvalidos = directorio + "\\Erroneos\\" + InfoArchivo.Name.Replace("EN_PROCESO_", "PROCESADO_CON_ERRORES_");
                moverArchivo(InfoArchivo.FullName, pathArchivosInvalidos);
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, decodificarArchivo, El proceso de validación del archivo:" + pNomArchivo + "][Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]" + "[" + idLog + "]");
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado] Archivo " + InfoArchivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Erroneos" + "[" + idLog + "]");
            }
        }
        ...
        public bool validarArchivos(bool validaCopiadoFiles)
        {
            //string log = ThreadContext.Properties["log"].ToString();
            //string ip = ThreadContext.Properties["ip"].ToString();
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO][" + log + "][AltaEmpleado]  [Iniciando proceso de alta]");
            ...
            if (existeArchivo.Count > 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Archivo " + archivo.FullName.Replace("EN_PROCESO_", "") + " finalizo de procesar y se envía a carpeta \\Erroneos]");
                    return false;
                }
            }
            return false;
        ...
        private bool validarContenido(FileInfo pInfoArchivo)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                {
                    LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, ValidarContenido, La línea numero: " + (counter + 1) + " no tinen los 361 caracteres, \\LNAltaEmpleado.cs: línea 156]");
                    file.Close();
                    return false;
                }
                if (line.StartsWith("11"))
                {
                    ...
                    if (!resultHeaderA) {
                        file.Close();
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Error en validacion encabezado]");
                ...
                if (line.StartsWith("21"))
                {
                    ...
                    if (!resultDetalle) {
                        file.Close();
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Error en validacion datos]");
                ...
                if (line.StartsWith("92"))
                {
                    ...
                    if (!resultTrailerA) {
                        file.Close();
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Error en validacion pie archivo]");
            ...
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado Obteniendo contratos]");
        ...
        private bool validaHeaderA(string pLine)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validaHeaderA, hay un error en el Header A en el campo: " + identificacionReg + ", \\LNAltaEmpleado.cs:línea 213]");
                return false;
            }
            if (!rgxFecha.IsMatch(fechaEnvio))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validaHeaderA, hay un error en el Header A en el campo: " + fechaEnvio + ", \\LNAltaEmpleado.cs:línea 218]");
                return false;
            }
            if (!rgxFiller.IsMatch(filler))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validaHeaderA, hay un error en el Header A en el campo: " + filler + ", \\LNAltaEmpleado.cs:línea 223]");
                return false;
            }
        ...
        private bool validaHeaderB(string pLine)
        {
                ...
                && rgxFiller.IsMatch(filler))
            {
                ...
                {
                    LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validaHeaderB, La empleadora con clave " + claveEmpleadora + " no se encuentra dada de Alta, \\LNAltaEmpleado.cs:línea 249]");
                    result = false;
                }
            }
            else
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validaHeaderB, hay un error en el Header B con empleadora:" + claveEmpleadora + ", \\LNAltaEmpleado.cs:línea 243]");
            }
        ...
        private bool validarDetalle(string pLine, int pNumLinea, string pClaveCliente, string pFechaEnvio, string pClaveBIN)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " la clave empresa no cumple el formato, \\LNAltaEmpleado.cs:línea 308]");
                return false;
            }

            if (!rgxAlfaNumerico10.IsMatch(regCliente))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el numero de cliente no cumple el formato, \\LNAltaEmpleado.cs:línea 313]");
                return false;
            }
            if (!rgxNombre.IsMatch(nombre))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el nombre no cumple el formato: " + nombre + ", \\LNAltaEmpleado.cs:línea 318]");
                return false;
            }
            if (!rgxTelefono.IsMatch(telefono))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado] [validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el Telefono no cumple el formato: " + telefono + ", \\LNAltaEmpleado.cs:línea 328]");
                return false;
            }
            if (!rgxTipoTarjeta.IsMatch(tipoTarjeta))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el Tipo Tarjeta no cumple el formato: " + tipoTarjeta + "]");
                return false;
            }
            if (tipoTarjeta.Equals("A"))
            {
                ...
                {
                    LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " la TarjetaTitular no cumple el formato: " + tarjetaTitular + "]");
                    return false;
                }
            }
            ...
            if (pLine.Count() == 372)
            {
                ...
                {
                    LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " la clave subproducto no cumple el formato, \\LNAltaEmpleado.cs:línea 414]");
                    return false;
                }
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, Correos inválidos en la línea " + (pNumLinea + 1) + ", Mensaje: " + exp.Message + " TRACE: " + exp.StackTrace + "]");
                return false;
            }
        }
        ...
        private bool validarDetalleCredito(string pLine, int pNumLinea, string pClaveCliente, string pFechaEnvio, string pClaveBIN)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " la clave Subproducto no cumple el formato: " + claveSubproducto + "]");
                return false;
            }
            if (!rgxLimiteCredito.IsMatch(limiteCredito))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el Limite de Credito no cumple el formato: " + limiteCredito + "]");
                return false;
            }
            if (!rgxDiaCorte.IsMatch(diaCorte))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarDetalle, hay un error en la línea no. " + (pNumLinea + 1) + " el Dia de corte no cumple el formato: " + diaCorte + "]");
                return false;
            }
            if (tipoTarjeta.Equals("A"))
            {
        ...
        private bool validarTrailerB(string pLine, int pContadorAltas)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarTrailerB, hay un error en el Trailer B no corresponde el numero de Altas por empleadora:" + numAltas + ", \\LNAltaEmpleado.cs:línea 341]");
                return false;
            }
            if (!rgxFiller.IsMatch(filler))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarTrailerB, hay un error en el Trailer B en el filler:" + filler + ", \\LNAltaEmpleado.cs:línea 346]");
                return false;
            }
            return true;
        }
        ...
        private bool validarTrailerA(string pLine, int pContadorAltas, int pContadorEmpleadoras)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarTrailerA, hay un error en el Trailer A no corresponde el número de Altas:" + numMovimientos + ", \\LNAltaEmpleado.cs:línea 367]");
                return false;
            }
            if (!(rgxFiller.IsMatch(filler)))
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, validarTrailerA, hay un error en el Trailer A no corresponde el filler:" + filler + "]");
                return false;
            }
        ...
        private bool insertaDatos(string pNomArchivo, bool esCredito = false, Guid? idLog = null)
        {
            ...
            {
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Bulk info]");
                ...
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Actualizando info bulk]");
            ...
            if (tmpDatosInsert != null)
            {
                ...
                {
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Creando tarjeta]");
                    if (tipoManuFactura.ToUpper().Equals("_V_"))
                    {
        ...
        private bool enviaDatosCacao(DataTable pDtContenidoFile, string pIdArchivo, string nomFile, DataTable tContenidoFileProducto)
        {
            ...
            try
            {
                ...
                for (int contador = 0; contador < pDtContenidoFile.Rows.Count; contador++)
                {
                    ...
                    try
                    {
                        ...
                        if (row["ID_EstatusCACAO"].ToString() != "2")
                        {
                            ...
                            if (respValidarTitular)
                            {
                                ...
                                xm = new XmlDocument();
                                xm.LoadXml(response);
                                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [WS_AltaEmpleado.CACAO.Prospectación] [RECIBIDO] [enviaDatosEvertec] [NOMBRE:" + xm.GetElementsByTagName("Nombre")[0].InnerText + ";FECHA:"
                                ...
                                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Tarjeta generada]");
                            ...
                            else
                            {
                                ...
                                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [WS_AltaEmpleado.CACAO.Prospectación] [validarTarjetaTitular] [Codigo: " + respValidarTarjetaTitular.Rows[0]["Codigo"].ToString() +
                    ...
                    {
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [WS_AltaEmpleado.CACAO.Prospectación, enviaDatosCACAO, Resulto un error al convertir la respuesta " + response + " en XML:" + ex.Message + ", " + ex.StackTrace + "]");
                    }
                }
            }
            catch (Exception ex)
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [WS_AltaEmpleado.CACAO.Prospectación, enviaDatosCACAO, Error al guardar respuesta CACAO, Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
            }
            finally
            {
                connection.Close();
        ...
        private bool enviarDatosParabilia(string pIdArchivoEntrada, string nomArchivo)
        {
            //string log = ThreadContext.Properties["log"].ToString();
            //string ip = ThreadContext.Properties["ip"].ToString();
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Guardabdo info tarjeta]");
            ...
            try
            {
                ...
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [enviarDatosParabilia, con Id de Archivo: " + pIdArchivoEntrada + "]");
                ...
                {
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [El SP procnoc_travel_ObtieneRegistrosCACAOAltas trae " + dtDatosProcesar.Rows.Count + " registros]");
                    for (int contador = 0; contador < dtDatosProcesar.Rows.Count; contador++)
                    {
                        ...
                        if (procesoFechaVencimiento)
                        {
                            ...
                            catch (Exception exp)
                            {
                                ...
                                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Error en la fecha de vencimiento: " + exp.Message + ", TRACE: " + exp.StackTrace + "]");
                                //Logueo.Evento("[Eror en la fecha de vencimiento: " + exp.Message + "] [TRACE: " + exp.StackTrace + "]");
                            }
                        }
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + " Inicio de Proceso de Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + "]");
                        ...
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, ENVIADO, enviarDatosParabilia, " + datosSalida + ", ", "PROCNOC", false + "]");
                        ...
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia,RECIBIDO, enviarDatosParabilia, CODIGO:" + codigo + ";RESPUESTA:" + descripcion + ", ", "PROCNOC", true + "]");
                        actualizaRegistrosBDPRocesadorCacaoAltas(codigo, paramParabilia.IdArchivoDetalle, paramParabilia.ClaveColectiva);
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + " Fin de Proceso de Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + "]");
                    }
                }
                ...
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + " Fin de Proceso de Archivo: " + nomArchivoProcesar + "]");
                return crearArchivoSalida(fechaVencimiento, pIdArchivoEntrada);
            }
            catch (Exception ex)
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, enviarDatosParabilia, Error al guardar datos parabilia:" + datosSalida + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
                return false;
            }
            finally
            {
        ...
        internal bool enviarDatosParabiliaCredito(string pIdArchivoEntrada, string nomArchivo, Guid? idLog)
        {
            ...
            try
            {
                Hashtable htProcesar = new Hashtable();
                htProcesar.Add("@idArchivo", pIdArchivoEntrada);
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [enviarDatosParabilia, con Id de Archivo: " + pIdArchivoEntrada + ", " + idLog + "]");
                ...
                if (dtDatosProcesar != null)
                {
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [El SP procnoc_travel_ObtieneRegistrosCACAOAltasCredito trae " + dtDatosProcesar.Rows.Count + " registros," + "," + idLog + "]");
                    for (int contador = 0; contador < dtDatosProcesar.Rows.Count; contador++)
                    {
                        var row = dtDatosProcesar.Rows[contador];
                        numTarjeta = row["Tarjeta"].ToString();
                        ...
                        if (procesoFechaVencimiento)
                        {
                            ...
                            catch (Exception exp)
                            {
                                DateTime fechainicio = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                                fechaVencimiento = fechainicio.AddMonths(1).AddDays(-1).AddYears(5).ToString("yyyyMMdd");
                                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Eror en la fecha de vencimiento: " + exp.Message + ", TRACE: " + exp.StackTrace + "," + "," + idLog + "]");
                            }
                        }
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + ", Inicio de Proceso de Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + ", " + idLog + "]");
                        ParamParabiliaAltas paramParabilia = new ParamParabiliaAltas()
                        {
                            ClaveColectiva = ((string.IsNullOrEmpty(row["ClaveEmpleadora"].ToString().TrimEnd(' '))
                                               ? row["ClaveEmisor"].ToString().TrimEnd(' ')
                        ...
                        datosSalida = new JavaScriptSerializer().Serialize(paramParabilia);
                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, ENVIADO, enviarDatosParabilia, " + datosSalida + "," + "," + idLog + ",", "PROCNOC", false + "]");
                        ...
                        if ((respuestaSubproducto == "correcto") && (credito == "0") && manufacturaCorrecta)
                        {
                            ...
                            using (transaccionSQL)
                            {
                                ...
                                if (codigo == "00")
                                {
                                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + ",  Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + " insertado en parabilia" + "," + idLog + "]");
                                    ...
                                    {
                                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + ",  Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + " creacion correcta del corte" + "" + idLog + "]");
                                        ...
                                        if (Convert.ToDecimal(paramParabilia.LimiteCredito) > 0)
                                        {
                                            ...
                                            if (codigoAsignaLimiteCredito != "error")
                                            {
                                                ...
                                                try
                                                {
                                                    ...
                                                    {
                                                        LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, enviarDatosParabilia, ID_ARCHIVO:" + pIdArchivoEntrada + ",  Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + " asignacion correcta de limite credito]");
                                                        //todo correcto
                                                        rollback = false;
                        ...
                        else
                        {
                            ...
                            {
                                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, enviarDatosParabilia, Manufactura incorrecta:" + "Registro " + (contador + 1) + " del Archivo: " + nomArchivoProcesar + "]");
                            }
                            ...
                            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado.SPparabilia, RECIBIDO, enviarDatosParabilia, CODIGO:" + codigoBusquedaSubProducto + ";RESPUESTA:" + descripcionBusquedaSubProducto + ", ", "PROCNOC", true + "]");
        ...
        private void actualizaRegistrosBDPRocesadorCacaoAltas(string pCodigo, string pIdArchivoDetalle, string pClaveColectiva)
        {
            ...
            try
            {
                if (pCodigo.Equals("00"))
                {
                    ...
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Se creo la colectiva: " + pClaveColectiva + "]");
                }
                else
                {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, actualizaRegistrosBDPRocesadorCacaoAltas, Error al actualizar el registro detalle con id: " + pIdArchivoDetalle + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
            }
        }
        ...
        private bool crearArchivoSalida(string fechaVencimiento, string pIdArchivoEntrada)
        {
            ...
            LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado] [ID_ARCHIVO:" + pIdArchivoEntrada + "] [Inicia Generación de Archivo de Respuesta del Archivo" + nomArchivoProcesar + " con nombre: " + nombreArchivoSalida + "]");
            ...
            try
            {
                ...
                for (int contador = 0; contador < datosSalida.Rows.Count; contador++)
                {
                    ...
                    {
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, ID_ARCHIVO:" + pIdArchivoEntrada + ", AltaEmpleado, crearArchivoSalida, No se pudo obtener la clave del tipo CACAO: " + pIdArchivoEntrada + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
                    }
                    try
                    {
                    ...
                    {
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, crearArchivoSalida, No se pudo obtener la CLABE: " + pIdArchivoEntrada + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
                    }
                ...
                if (validaError)
                {
                    ...
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, Archivo " + pathArchivosInvalidos + " finalizo de procesar y se envía a carpeta \\Erroneos]");
                }
                else
                {
                    LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, ID_ARCHIVO:" + pIdArchivoEntrada + " Finaliza Generación de Archivo de Respuesta del Archivo" + nomArchivoProcesar + " con nombre: " + nombreArchivoSalida + "]");
                }
            ...
            {
                streamSalida.Close();
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, crearArchivoSalida, Error al crear el archivo de salida con Identificador: " + pIdArchivoEntrada + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
                return false;
            }
        }
        ...
        private bool crearArchivoSalidaCredito(string fechaVencimiento, string pIdArchivoEntrada)
        {
            ...
            try
            {
                ...
                for (int contador = 0; contador < datosSalida.Rows.Count; contador++)
                {
                    ...
                    {
                        cuentaArchivoSalida = "";
                        LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, crearArchivoSalida, No se pudo obtener la clave del tipo CACAO: " + pIdArchivoEntrada + ", Mensaje: " + ex.Message + " TRACE: " + ex.StackTrace + "]");
                    }
                    try
                    {
                        cuentaClabe = BDsps.obtieneClaveMACacaoFromTarjeta(SeguridadCifrado.descifrar(row["Tarjeta"].ToString()), "CLABE");
        ...
        private string obtenerFechaVencimiento(string pNumTarjeta)
        {
            ...
            try
            {
                ...
                xm.LoadXml(response);
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [WS_AltaEmpleado.Evertec.ConsultaVigencia, RECIBIDO, obtenerFechaVencimiento, " + response.Replace("\n", " ").Replace("\r", " ") + ",", "PROCNOC", true + "]");
            ...
            catch (Exception exp)
            {
                ...
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [Eror en la fecha de vencimiento: " + exp.Message + ", TRACE: " + exp.StackTrace + "]");
        ...
        private void enviarCorreoAlta(string correo, string mailCFDI, string nombre, string ClaveColectiva)
        {
            ...
            if (datosSalida.Rows.Count > 0)
                ...
                LogueoAltaEmpleado.Info("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado,  No se obtuvo el Nombre a Embozar de la Colectiva padre]");
        ...
        private string construirBody(string correo, string mailCFDI, string nombre, string nomEmpresa)
        {
            ...
            {
                LogueoAltaEmpleado.Error("[" + ip + "] [TarjetasNominativas] [PROCESADORNOCTURNO] [" + log + "] [AltaEmpleado, construirBody, El archivo html para el cuerpo del correo:" + correo + " no se encuentra, Mensaje: " + exp.Message + " TRACE: " + exp.StackTrace + "]");
                return null;
            }
