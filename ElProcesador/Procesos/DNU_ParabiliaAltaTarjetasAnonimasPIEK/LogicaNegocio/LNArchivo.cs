...
using DNU_ParabiliaAltaTarjetasAnonimasPIEK.BaseDatos;
using System.Threading;
using DNU_ParabiliaAltaTarjetasAnonimasPIEK.Utilidades;
using log4net;

namespace DNU_ParabiliaAltaTarjetasAnonimasPIEK.LogicaNegocio
{
    public class LNArchivo
    {
        ...
        public static Boolean ProcesaArch(String path, String claveColectiva
                    , string tipoManuFactura, ref List<Archivo> lstArchvio
                    , int longitudBin
                    , string subproducto = null, bool subproductoValido = false
                    , bool esCredito = false, string log = "", string ip = ""
                    , int longitudBinValida = 1)
        {
            try
            {
                ...
                if (longitudBinValida == 0)
                {
                    LogueoPrAltaTarjParabilia.Error("[" + ip + "] [ProcesarAltaTarjetas] [PROCESADORNOCTURNO] [" + log + "] [NO ESTA DEFINIDO EL PARAMETRO @LongitudBIN]");
                    throw new Exception("NO ESTA DEFINIDO EL PARAMETRO @LongitudBIN");
                }
                ...
                var ejecucionCorrecta = ejecutaActivacion(ref lstArchvio, ref sb, claveColectiva, tipoManuFactura
                                            , longitudBin, subproducto, subproductoValido, esCredito, log, ip);
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < files.Length; i++)
                    {
                        ...
                        if (subproducto != "")
                        {
                            ...
                            if (!(resultadoConsultaSubproducto is null))
                            {
                                if (resultadoConsultaSubproducto.Rows[0]["tipo"].ToString() == "correcto")
                                {
                                    ...
                                    LogueoPrAltaTarjParabilia.Debug("[" + ip + "] [ProcesarAltaTarjetas] [PROCESADORNOCTURNO] [" + log + "] [Subproducto validado correctamente]");
                                    longitudBinValida = Convert.ToInt32(resultadoConsultaSubproducto.Rows[0]["longitudValida"]);
                                    longitudBin = Convert.ToInt32(resultadoConsultaSubproducto.Rows[0]["longitudBin"]);
                                }
                        ...
                        resp2 = LNArchivo.ProcesaArch(elpathFile, claveColectiva
                                    , tipoManufactura, ref lstArchvio, longitudBin
                                    , subproducto, subproductoValido, esCredito
                                    , log, ip, longitudBinValida);
        ...
        private static bool ejecutaActivacion(ref List<Archivo> lstArchvio, ref StringBuilder sb
                        , String claveColectiva, string tipoManufactura, int longitudBin
                        , string subproducto = null, bool subproductoValido = false
                        , bool esCredito = false, string log = "", string ip = "")
        {
                        ...
                        using (SqlTransaction transaccionSQL = conn.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (datosArchivo[1] != "00000")
                                {
                                    ...
                                    resultado = DAOArchivo.AltaTarjeta(item, claveColectiva, tipoManufactura, "", longitudBin, false, null, conn, transaccionSQL, subproducto);// DBProcesadorArchivo.strBDEscrituraAutorizador);DBProcesadorArchivo.strBDEscrituraAutorizador
                                    if (resultado[0] == "00")
                                    {
                                        try
                                        {
        ...
        internal static string LimpiarArchivo(String elArchivo, string path, string log, string ip)
        {
            ...
            using (StreamReader sr = new StreamReader(elArchivo, Encoding.UTF8))
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (Regex.Match(line, @"^[0-9]{8}[\s]{17}[0-9]{16}").Success)// @"^[0-9]{8}[\s][0-9]{13,15}[\s]").Success)
                    {
