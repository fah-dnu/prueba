...
namespace DNU_MovimientoManualMasivo.BaseDatos
{
    public class DAODatosBase
    {
            ...
            , String ClaveEvento, String elUsuario, Response respuesta, string ip, string log, SqlConnection conn = null, SqlTransaction transaccionSQL = null)
        {
            try
            {
                LogueoMovimientoMasivo.Info("[" + ip + "] [MovimientoMasivo] [PROCESADORNOCTURNO] [" + log + "] [Ejecuta SP ws_Ejecutor_ObtieneValoresContratos]");
        ...
        public static DataSet obtenerDatosTarjetaEventoManualMasivoParaExecute(string numeroTarjeta
            , string importe, SqlConnection pConn, string ip, string log)
        {

            LogueoMovimientoMasivo.Info("[" + ip + "] [MovimientoMasivo] [PROCESADORNOCTURNO] [" + log + "] [Ejecuta SP ProcNoct_ObtenerDatosMovimientoManualMasivo]");
            SqlCommand query = new SqlCommand("ProcNoct_ObtenerDatosMovimientoManualMasivo", pConn);
            query.CommandType = CommandType.StoredProcedure;
            ...
            query.Parameters.AddWithValue("@claveEvento", PNConfig.Get("MOVIMIENTO_MASIVO", "EventoMasivo"));
            query.Parameters.AddWithValue("@importe", importe);
                                    ...
                                    SqlTransaction transaccionSQLTraspaso, SqlConnection conn, string ip, string log)
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        polizaCreada = true;
                        LogueoMovimientoMasivo.Info("[" + ip + "] [MovimientoMasivo] [PROCESADORNOCTURNO] [" + log + "] [Se Realizó la Bonificación de Puntos a la Cuenta de la Colectiva: " + elAbono.IdColectiva.ToString() + "]");
                    }
                }
            ...
            {
                LogueoMovimientoMasivo.Error("[" + ip + "] [MovimientoMasivo] [PROCESADORNOCTURNO] [" + log + "] [RegistraEvManual_AbonoPuntos() " + err.Message + "]");
        ...
        public static DataSet GuardarArchivo(string pathOriginal, string pathFinal, List<DMovimientoMasivo> listaRegistros)
        {
            using (SqlConnection conn = new SqlConnection(DBProcesadorArchivo.strBDEscrituraBatch))
            {
                ...
                query.Parameters.AddWithValue("@descripcion", "Archivo de Movimiento Manual Masivo");
                query.Parameters.AddWithValue("@claveProceso", "MOVIMIENTO_MASIVO");
